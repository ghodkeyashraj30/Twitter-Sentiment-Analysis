...
function_option = st.sidebar.selectbox("Select The Funtionality: ", ["Search By Username","Search By #Tag and Words"])
...
st.info("1 Tweets takes approx 0.02 sec so you may have to wait {} minute for {} Tweets, So Please Have Patient.".format(round((number_of_tweets*0.02/60),2), number_of_tweets))
...
if st.button("Analysis Sentiment"):
    ...
    st.write(data)
    download_data(data, label="Twitter Sentiment Filtered")
    st.write(" ")
    ...
    st.line_chart(analyse)
    st.area_chart(analyse)
